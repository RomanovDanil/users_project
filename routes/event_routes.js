...
router.post(
    ...
    try{
        ...
        const {event_title, start_date, c_1_date, c_plus_1_date, finish_date} = req.body
        
        //проверить наличие события в базе
        const curEvent = await Event.findOne({event_title})
        if(curEvent){
            return res.status(400).json({message:'Event with current title is exists.'})
        }

        if(start_date >= Date.now){
            return res.status(400).json({message:'You cant create an event that has already ended.'})
        }   

        //проверить правильность введенных дат
        const date_errors = await isCorrectDates(start_date, c_1_date, c_plus_1_date, finish_date)
        if(date_errors.count > 0){
            return res.status(400).json({message:date_errors})
        }

        const event = new Event({event_title, start_date, c_1_date, c_plus_1_date, finish_date})
...
async function isCorrectDates(start_date, c_1_date, c_plus_1_date, finish_date){
    date_errors = array()
...
router.put('/update', async (req, res) => {
    try{
        ...
        if(!event){
            ...
            return res.status(200).json({message:"Event's data is successfully updated"})
        }
    } catch(e) {
        res.status(500).json({message:e.message})
    } 
